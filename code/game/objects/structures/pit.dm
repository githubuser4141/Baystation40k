...
/obj/structure/pit/use_tool(obj/item/tool, mob/user, list/click_params)
	...
	if (istype(tool, /obj/item/shovel))
		...
		if (!user.do_skilled((tool.toolspeed * 5) SECONDS, SKILL_VIGOR, src) || !user.use_sanity_check(src, tool))
			return TRUE
		user.visible_message(
...
/obj/structure/gravemarker/use_tool(obj/item/tool, mob/user, list/click_params)
	...
	if (isHatchet(tool))
		...
		if (!user.do_skilled((tool.toolspeed * 2) SECONDS, list(SKILL_CONSTRUCTION, SKILL_VIGOR), src) || !user.use_sanity_check(src, tool))
			return TRUE
