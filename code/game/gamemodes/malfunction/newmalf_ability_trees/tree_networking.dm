...
/datum/game_mode/malfunction/verb/advanced_encryption_hack()
	...
	if(user.last_failed_malf_title || user.last_failed_malf_message)
		if (alert(user, "Your last hack attempt with title '[user.last_failed_malf_title]' has failed. Try again?", "Retransmission", "Compliance", "No") != "Compliance")
...
/datum/game_mode/malfunction/verb/system_override()
	...
	var/price = 500
	var/mob/living/silicon/ai/user = usr
	if (alert(user, "Begin system override? This cannot be stopped once started. The network administrators will probably notice this.", "System Override:", "Compliance", "No") != "Compliance")
		return
