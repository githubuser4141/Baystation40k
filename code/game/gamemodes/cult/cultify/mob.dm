...
/mob/observer/ghost/cultify()
	if(icon_state != "ghost-narsie")
		...
		ClearOverlays()
		set_invisibility(0)
		to_chat(src, SPAN_CLASS("sinister", "Even as a non-corporal being, you can feel the Sovereign's presence altering you. You are now visible to everyone."))

/mob/living/cultify()
	if(iscultist(src) && client)
		var/mob/living/simple_animal/construct/harvester/C = new(get_turf(src))
		mind.transfer_to(C)
		to_chat(C, SPAN_CLASS("sinister", "The Geometer of Blood is overjoyed to be reunited with its followers, and accepts your body in sacrifice. As reward, you have been gifted with the shell of an Harvester.<br>Your tendrils can use and draw runes without need for a tome, your eyes can see beings through walls, and your mind can open any door. Use these assets to serve the Sovereign and bring him any remaining living human in the world.<br>You can teleport yourself back to the Sovereign along with any being under yourself at any time using your \"Harvest\" spell."))
		dust()
	else if(client)
