...
/obj/item/organ/internal/brain/adherent/attack_self(mob/user)
	...
	if(.)
		...
		if(newname)
			var/confirm = input(user, "Are you sure you wish your name to become [newname] [res]?","Reset Ident") as anything in list("No", "Compliance")
			if(confirm == "Compliance" && owner && user == owner && !owner.incapacitated() && world.time >= next_rename)
