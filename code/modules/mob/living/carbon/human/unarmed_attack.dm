...
/datum/unarmed_attack/proc/show_attack(mob/living/carbon/human/user, mob/living/carbon/human/target, zone, attack_damage)
	...
	user.visible_message(SPAN_WARNING("\The [user] [pick(attack_verb)] \the [target] in the [affecting.name]!"))
...
/datum/unarmed_attack/proc/handle_eye_attack(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(eyes)
		...
		user.visible_message(SPAN_DANGER("\The [user] presses [user_pronouns.his] [eye_attack_text] into \the [target]'s [eyes.name]!"))
		...
		return
	user.visible_message(SPAN_DANGER("\The [user] attempts to press [user_pronouns.his] [eye_attack_text] into \the [target]'s eyes, but they don't have any!"))
...
/datum/unarmed_attack/punch/show_attack(mob/living/carbon/human/user, mob/living/carbon/human/target, zone, attack_damage)
	...
	if(target == user)
		...
		user.visible_message(SPAN_DANGER("\The [user] [pick(attack_verb)] [pronouns.self] in the [organ]!"))
		return FALSE
		...
		switch(zone)
			if(BP_HEAD, BP_MOUTH, BP_EYES)
				...
				switch(attack_damage)
					if(1 to 2)
						user.visible_message(SPAN_DANGER("\The [user] slapped \the [target] across [target_pronouns.his] cheek!"))
					if(3 to 4)
						user.visible_message(pick(
							80; SPAN_DANGER("\The [user] [pick(attack_verb)] \the [target] in the head!"),
							20; SPAN_CLASS("danger", "\The [user] struck \the [target] in the head[pick("", " with a closed fist")]!"),
							50; SPAN_DANGER("\The [user] threw a hook against \the [target]'s head!")
							))
					if(5)
						user.visible_message(pick(
							10; SPAN_DANGER("\The [user] gave \the [target] a solid slap across [target_pronouns.his] face!"),
							90; SPAN_CLASS("danger", "\The [user] smashed [user_pronouns.his] [pick(attack_noun)] into \the [target]'s [pick("[organ]", "face", "jaw")]!")
							))
			else
				// ----- BODY ----- //
				switch(attack_damage)
					if(1 to 2)	user.visible_message(SPAN_DANGER("\The [user] threw a glancing punch at \the [target]'s [organ]!"))
					if(1 to 4)	user.visible_message(SPAN_DANGER("\The [user] [pick(attack_verb)] \the [target] in [target_pronouns.his] [organ]!"))
					if(5)		user.visible_message(SPAN_DANGER("\The [user] smashed [user_pronouns.his] [pick(attack_noun)] into \the [target]'s [organ]!"))
	else
...
/datum/unarmed_attack/stomp/show_attack(mob/living/carbon/human/user, mob/living/carbon/human/target, zone, attack_damage)
	...
	switch(attack_damage)
		if(1 to 4)
			user.visible_message(pick(
				SPAN_CLASS("danger", "\The [user] stomped on \the [target]'s [organ][pick("", "with their [shoe_text]")]!"),
				SPAN_DANGER("\The [user] stomped [user_pronouns.his] [shoe_text] down onto \the [target]'s [organ]!")))
		if(5)
			user.visible_message(pick(
