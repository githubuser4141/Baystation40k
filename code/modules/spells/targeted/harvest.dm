	...
	if(destination)
		...
		for(var/mob/living/M in targets)
			if(!findNullRod(M))
				...
				if(M != user)
					prey = 1
		to_chat(user, SPAN_CLASS("sinister", "You warp back to the Sovereign[prey ? " along with your prey":""]."))
	else
		to_chat(user, SPAN_DANGER("...something's wrong!"))//There shouldn't be an instance of Harvesters when the Sovereign isn't in the world.
