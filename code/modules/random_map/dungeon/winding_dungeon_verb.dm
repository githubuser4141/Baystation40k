...
/client/proc/create_dungeon()
	...
	if(alert("Would you like to modify generated room/corridor variables?",,"No","Compliance") == "Compliance")
	...
	if(alert("Would you like to modify generational multipliers?",,"No","Compliance") == "Compliance")
	...
	while(1)
		...
		switch(mode)
			if("room_theme")
				if(alert("Would you like to modify what room layouts are used?",,"No","Compliance") != "Compliance")
					mode = "monsters"
					continue
			if("monsters")
				if(alert("Would you like to modify what mobs get spawned?",,"No","Compliance") != "Compliance")
					mode = "loot"
					continue
			if("loot")
				if(alert("Would you like to modify what loot gets spawned?",,"No","Compliance") !="Compliance")
					break
		...
		while(1)
			...
			if(!layout)
				if(current_list == common && alert("Would you like to add items to the uncommon pool?",,"No","Compliance") == "Compliance")
					current_list = uncommon
					continue
				else if(current_list != rare && alert("Would you like to add items to the rare pool?",, "No", "Compliance") == "Compliance")
					current_list = rare
					continue
				else
					break
			var/chance = input("What is the weight of this item? Higher weights get picked more frequently. Default is 10",10) as num
			if(alert("Do you want to add all subtypes of this item to the current pool as well? They will be weighted the same.",,"No","Compliance") == "Compliance")
				var/list/local_total = typesof(layout)
				if(alert("Do you want to only add the subtypes of this item?",,"No","Compliance") == "Compliance")
					local_total -= layout
	...
	if(alert("Are you sure you want to create this? It will appear at your position.",,"No","Compliance") != "Compliance")
		return
	...
	for(var/x in 1 to vars["limit_x"])
		for(var/y in 1 to vars["limit_y"])
			...
			if(!T)
				to_chat(src, SPAN_DANGER("Error, turf could not be located. Probably out of bounds."))
				return
