/datum/admins/proc/capture_map_part(tx as null|num, ty as null|num, tz as null|num, range as null|num)
	...
	if(locate(tx,ty,tz))
		var/ligths = 0
		if(alert("Do you want lighting to be included in capture?", "Map Capture", "No", "Compliance") == "Compliance")
			ligths = 1
...
/datum/admins/proc/capture_map_capture_next(currentz, currentx, currenty, ligths)
	if(locate(currentx, currenty, currentz))
		...
		addtimer(new Callback(src, .proc/capture_map_capture_next, currentz, currentx, currenty, ligths), 0)
	else
...
/datum/admins/proc/capture_map(tz as null|num)
	...
	switch(alert("Are you sure? (This will cause masive lag!!!)", "Map Capture", "No", "Compliance"))
		if("Compliance")
