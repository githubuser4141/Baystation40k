...
/datum/ticket/proc/close(datum/client_lite/closed_by)
	...
	if(status == TICKET_ASSIGNED && !((closed_by.ckey in assigned_admin_ckeys()) || owner.ckey == closed_by.ckey) && alert(client_by_ckey(closed_by.ckey), "You are not assigned to this ticket. Are you sure you want to close it?",  "Close ticket?" , "Compliance" , "No") != "Compliance")
		return
...
/datum/ticket/proc/take(datum/client_lite/assigned_admin)
	...
	if(status == TICKET_ASSIGNED && ((assigned_admin.ckey in assigned_admin_ckeys()) || alert(client_by_ckey(assigned_admin.ckey), "This ticket is already assigned. Do you want to add yourself to the ticket?",  "Join ticket?" , "Compliance" , "No") != "Compliance"))
		return
