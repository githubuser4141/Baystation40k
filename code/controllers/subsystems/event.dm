...
/datum/controller/subsystem/event/Topic(href, href_list)
	...
	if(href_list["toggle_report"])
		...
		if(alert("Stopping an event may have unintended side-effects. Continue?","Stopping Event!","Compliance","No") != "Compliance")
			return
		...
		if(alert("This will remove the event from rotation. Continue?","Removing Event!","Compliance","No") != "Compliance")
			return
		...
		if(!new_event.name || !new_event.event_type)
			return
		if(alert("This will add a new event to the rotation. Continue?","Add Event!","Compliance","No") != "Compliance")
			return
