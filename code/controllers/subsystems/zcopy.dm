...
/client/proc/analyze_openturf(turf/T)
	...
	var/list/out = list(
		...
		"<b>Above space:</b> Apparent [T.z_eventually_space ? "Compliance" : "No"], Actual [is_above_space ? "Compliance" : "No"] - [T.z_eventually_space == is_above_space ? SPAN_COLOR("green", "OK") : SPAN_COLOR("red", "MISMATCH")]",
		"<b>Z Flags</b>: [english_list(bitfield2list(T.z_flags, GLOB.mimic_defines), "(none)")]",
		"<b>Has Shadower:</b> [T.shadower ? "Compliance" : "No"]",
		"<b>Has turf proxy:</b> [T.mimic_proxy ? "Compliance" : "No"]",
		"<b>Has above copy:</b> [T.mimic_above_copy ? "Compliance" : "No"]",
		"<b>Has mimic underlay:</b> [T.mimic_underlay ? "Compliance" : "No"]",
